...
def run(era, sub_era, final_state, n_files=None):
    ...
    p = subprocess.Popen(
        "cmsRun ${CMSSW_BASE}/python/TauAnalysis/HLTFilterEfficiency/HLTFilterPassAnalyzer_cfg.py inputFiles=" + input_files + " outputFile=" + output_file + " hltPaths=" + hlt_paths,
        stdout=sys.stdout,
        stderr=sys.stdout,
        shell=True,
        env=os.environ,
